...
mask_path = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "files", "mask.json")
)
...
while True:
    ...
    if success:
        ...
        blue_mask = functions.generate_mask(
            masks["blue"]["max"], masks["blue"]["min"], hsv_image
        )
        yellow_mask = functions.generate_mask(
            masks["yellow"]["max"], masks["yellow"]["min"], hsv_image
        )
        green_mask = functions.generate_mask(
            masks["green"]["max"], masks["green"]["min"], hsv_image
        )
        ...
        contours_blue, hb = cv.findContours(
            canny_blue, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE
        )
        contours_yellow, hy = cv.findContours(
            canny_yellow, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE
        )
        contours_green, hb = cv.findContours(
            canny_green, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE
        )
        ...
        for cnt in contours_blue:
            ...
            if area > 300:
                ...
                cv.putText(
                    frame, "Azul", (x, y - 10), cv.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0)
                )
